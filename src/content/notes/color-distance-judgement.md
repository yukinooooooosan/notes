...
description: "RGB距離、OKLab距離、CIEDE2000を実装して、色の近さをゲームの勝敗判定として扱う方法を整理したメモ。"
pubDate: 2026-05-27
tags:
  - game
  - color
...
raw distanceが完全に同じ、または誤差範囲内なら同率にするのがよい。
...
そして、最小距離との差が `EPSILON` 以下なら同率1位として扱う。

RGB距離では同点が起きやすい。特に、白に対するCMY、黒に対するRGBのようなケースでは自然に同点になる。
